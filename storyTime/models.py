	...
	def __setitem__(self, key, value):
		if key in self.maps.names:
			try:
				...
				print('cant set {0} with value {1}'.format(key, value))
...
class RecorderModel(MappingModel):
	...
	@property
	def recording(self):
		return self.recordingModel.recordings[self._recordingIndex]
	
	@property
	def recordingIndex(self):
		return self._recordingIndex
	@recordingIndex.setter
	def recordingIndex(self, value):
		# TODO: validate index
		self._recordingIndex = value
		self.allMappingsChanged()
	...
	def hasRecording(self):
		return self.recording is not None and self.recording.duration > 0
