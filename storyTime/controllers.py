...
class StoryTimeWindow(object):
	...
	def __init__(self, parent=None):
		...
		self.recorderView.setRecorderModel(self._recorderModel)
		self.recorderView.setRecordingModel(self._recordingModel)
		...
		# subscribe recorder to the recording view selection change
		self.recordingView.selectionChanged.connect(self.recorderView.setRecordingIndex)
	...
	def fileMenuAboutToShow(self):
		self.ui.actionSaveRecordingAs.setEnabled(self._recorderModel.hasRecording)
		self.ui.actionExportMovie.setEnabled(self._recorderModel.hasRecording)
		self.ui.actionExportForEditing.setEnabled(self._recorderModel.hasRecording)
	
	def openStoryTimePath(self):
		dir_ = utils.getStoryTimePath()
	...
	def openRecording(self):
		...
		f = QFileDialog.getOpenFileName(
			self.ui,
			caption=caption,
			dir=utils.getStoryTimePath(),
			filter='XML files (*.xml)',
		)[0]
		self._recordingModel.openRecording(f)
...
class RecorderView(QWidget):
	...
	def __init__(self, parent=None):
		...
		self._recDisplayDataMapper = QDataWidgetMapper()
	...
	def setRecordingIndex(self, row):
		self._recorderModel.recordingIndex = row
		self._recDisplayDataMapper.setCurrentIndex(row)

	def record(self):
		if self._recorderModel.isRecording or self._recorderModel.isPlaying:
			self.stop()
		else:
			self._recorderModel.record()
	
	def play(self):
		self._recorderModel.play()
	
	def stop(self):
		self._recorderModel.stop()
	...
	def recordFrame(self, imagePath=None):
		if imagePath is None:
			self._recorderModel.recordCurrentFrame
		else:
			self._recorderModel.recordFrame(imagePath)
	
	def toggleTimeDisplay(self):
		self._displayDataMapper.itemDelegate().toggleTimeDisplay()
		self._displayDataMapper.revert()
	
	def setRecorderModel(self, model):
		self._recorderModel = model
	...
	def setRecordingModel(self, model):
		self._recordingModel = model
		
		self._recDisplayDataMapper.setModel(model)
		self._recDisplayDataMapper.setItemDelegate(RecordingViewDisplayDelegate(self))
		self._recDisplayDataMapper.addMapping(self.ui.name, model.maps.name)
		self._recDisplayDataMapper.addMapping(self.ui.duration, model.maps.duration)
		self._recDisplayDataMapper.addMapping(self.ui.imageCount, model.maps.imageCount)
		self._recDisplayDataMapper.addMapping(self.ui.fps, model.maps.fps)
		self._recDisplayDataMapper.toFirst()
...
class RecorderViewDisplayDelegate(QItemDelegate):
	...
	def setEditorData(self, editor, index):
		m = index.model()
		if index.column() == m.maps.frame:
			fps = m.index(column=m.maps.fps).data()
			if fps is not None:
				time = index.data()
		...
		elif index.column() == m.maps.timelineDuration:
			fps = m.index(column=m.maps.fps).data()
...
class RecordingViewDisplayDelegate(QItemDelegate):
	"""
	...
	def setEditorData(self, editor, index):
		m = index.model()
		if index.column() == m.maps.name:
			editor.setText(index.data())
		
		elif index.column() == m.maps.duration:
			m = index.model()
			fps = m.index(index.row(), column=m.maps.fps).data()
			if fps is not None:
				time = utils.getTimecode(int(index.data() * fps), fps)
				editor.setText(time)
		
		elif index.column() == m.maps.imageCount:
			editor.setText(self.imageCountLabel(index.data()))
		
		elif index.column() == m.maps.fps:
